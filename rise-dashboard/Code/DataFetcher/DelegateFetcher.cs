        ...
        public static async Task<DelegateResult> FetchDelegates()
        {
            try
            {
                ...
                using (var hc = new HttpClient())
                {
                    ...
                    var result200to399 = JObject.Parse(await hc.GetStringAsync(AppSettingsProvider.APIUrl + "/api/delegates?offset=101"));
                    var delegate200to399 = JsonConvert.DeserializeObject<DelegateResult>(result200to399.ToString());

                    var result400to599 = JObject.Parse(await hc.GetStringAsync(AppSettingsProvider.APIUrl + "/api/delegates?offset=202"));
                    var delegate400to599 = JsonConvert.DeserializeObject<DelegateResult>(result400to599.ToString());

                    var result600to799 = JObject.Parse(await hc.GetStringAsync(AppSettingsProvider.APIUrl + "/api/delegates?offset=303"));
                    var delegate600to799 = JsonConvert.DeserializeObject<DelegateResult>(result600to799.ToString());

                    var result800to999 = JObject.Parse(await hc.GetStringAsync(AppSettingsProvider.APIUrl + "/api/delegates?offset=404"));
